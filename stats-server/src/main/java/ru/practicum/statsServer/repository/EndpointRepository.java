...
public interface EndpointRepository extends JpaRepository<Endpoint, Long> {
    @Query("select new ru.practicum.statsServer.model.dto.EndpointDtoOutput(en.app,en.uri,count (en.id)) " +
            "from Endpoint as en where en.timestamp between :start and :end group by en.app,en.uri")
    List<EndpointDtoOutput> findAllByTimestampBetween(LocalDateTime start, LocalDateTime end);

    @Query("select new ru.practicum.statsServer.model.dto.EndpointDtoOutput(en.app,en.uri,count (en.id)) " +
            "from Endpoint as en where en.timestamp between :start and :end group by en.app,en.uri, en.id")
    ...
    @Query("select new ru.practicum.statsServer.model.dto.EndpointDtoOutput(en.app,en.uri,count (en.id)) " +
            "from Endpoint as en where en.timestamp between :start and :end and en.uri in :uris group by en.app,en.uri")
